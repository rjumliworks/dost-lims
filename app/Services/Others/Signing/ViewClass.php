...
namespace App\Services\Others\Signing;
...
class ViewClass
{
    ...
    public function normalizeOnly($request)
    {
        $request->validate([
        ...
        if (!$response->successful()) {
            return [
                'data' => [],
                'message' => 'Failed to normalize the PDF',
                'info' => 'Normalization service error.',
                'status' => false,
            ];
        }
        ...
        return [
            ...
            'info' => 'The PDF has been normalized successfully.',
        ];
    }

    public function signOnly($request)
    {
        $request->validate([
            'pdf' => 'required|file|mimes:pdf',
            'field_name' => 'required|string',
            'page_number' => 'required|integer',
            'box_x0' => 'required|numeric',
            'box_y0' => 'required|numeric',
            'box_x1' => 'required|numeric',
        ...
        if (!$user->certificate) {
            return [
                ...
                'info' => 'No PNPKI certificate found for your account.',
                'status' => false,
            ];
        }
        ...
        if (!$response->successful()) {
            return [
                'data' => [],
                'message' => 'Failed to sign the PDF',
                'info' => $response->json('message') ?? 'Signing service error.',
                'status' => false,
            ];
        }
        ...
        return [
            'data' => base64_encode($response->body()),
            'message' => 'PDF signed.',
            'info' => 'The PDF has been signed successfully.',
        ];
    }

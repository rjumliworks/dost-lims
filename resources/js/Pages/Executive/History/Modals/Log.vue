<template>
    <b-modal v-model="showModal" style="--vz-modal-width: 650px;" header-class="p-3 bg-light" title="View Change" class="v-modal-custom" modal-class="zoomIn" centered no-close-on-backdrop>
                    ...
                    <div class="col-md">
                        <div>
                            ...
                            <div class="hstack gap-3 fs-12 flex-wrap">
                                <div v-if="selected.tsr_code">TSR :
                                    <span class="fw-medium">{{ selected.tsr_code }}</span>
                                </div>
                                ...
                                <div class="vr" style="width: 1px;"></div>
                                <div>User :
                                    <span class="fw-medium">{{ selected.causer?.name || '—' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="row mt-2">
            <div class="col-sm-12">
                <table class="table table-nowrap table-bordered align-middle mb-0">
                    ...
                    <tbody v-if="changedFields.length > 0">
                        <tr v-for="(item, index) in changedFields" :key="index">
                            <td class="text-center text-capitalize">{{ item.field.replace(/_/g, ' ') }}</td>
                    ...
                    <tbody v-else>
                        <tr>
                            <td colspan="3" class="text-center text-muted">No field changes recorded.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
